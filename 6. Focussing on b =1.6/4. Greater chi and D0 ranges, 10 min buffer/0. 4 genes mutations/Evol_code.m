...
for i = 1:predpopsize
	  %         preyfit(i,j) = (-sqrt(mean(preyconstate(i,:).*avpreyawakestate(j,:)))+a*sqrt(mean(preyconstate(i,:).*avpredawakestate(j,:))))*(b*sqrt(preyusefulawake(i,j))+ sqrt(preyusefulsleep(i,j)));
%         predfit(i,j) = -sqrt(mean(predconstate(i,:).*avpredawakestate(j,:)))*(sqrt(predusefulsleep(i,j))+c*sqrt(mean(predconstate(i,:).*avpreyawakestate(j,:)))); %this is to be minimised
