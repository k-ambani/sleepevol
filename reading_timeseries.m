%% population level awake states only
k =26; %coefficient set
...
gen_values = [194, 207, 220, 233, 247];
%
genes = [9,13,17,21];
...
% end_gen = 275;
% gen_values = floor(linspace(start_gen,end_gen,figcols));
gen_values = [235, 240,245,255, 260];
%
genes = [9,13,17,21];
gene_names = {'D0','chi','a','tau'};
...
% filenames string
filenames = {'_avpreyavawakestate.xlsx','_avpredavawakestate.xlsx', '_fittestpreyawakestate.xlsx','_fittestpredawakestate.xlsx','_preyvecsum.csv','_predvecsum.csv'};
all_files = [];
...
toc 
vecsumprey = csvread(char(all_files(5)));
fprintf('vecsumprey done\n')
toc
vecsumpred = csvread(char(all_files(6)));
fprintf('vecsumpred done\n')
toc
